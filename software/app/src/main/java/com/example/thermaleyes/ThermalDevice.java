...
public abstract class ThermalDevice {
    ...
    private final UsbSerialInterface.UsbReadCallback mCallback =
            arg0 -> {
                try {
                    ...
                    if (tempData == null || tempData.length < IMAGE_PIXEL) {
                        return;
                    }
                    ...
                    int c1 = 11 * IMAGE_WIDTH + 15;
                    int c2 = 11 * IMAGE_WIDTH + 16;
                    int c3 = 12 * IMAGE_WIDTH + 15;
                    int c4 = 12 * IMAGE_WIDTH + 16;
                    float centerTemp = (tempData[c1] + tempData[c2] + tempData[c3] + tempData[c4]) / 4.0f;

                    ByteBuffer tempImage = getTemperatureImage(tempData, maxTemp, minTemp);
                    onFrame(tempImage, maxTemp, minTemp, centerTemp, maxLoc, minLoc);
                } catch (Throwable t) {
                    Log.e(TAG, "Thermal data parsing error: " + t.getMessage());
                }
            };

    public abstract void onFrame(ByteBuffer frame, float maxTemp, float minTemp, float centerTemp, int maxLoc, int minLoc);
    ...
    public synchronized boolean isConnected() {
        return mSerial != null && mSerial.isOpen();
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    public synchronized void connect() throws IOException {
        if (mSerial != null && mSerial.isOpen()) {
            Log.i(TAG, "ThermalDevice already connected, skipping redundant connect");
            return;
        }
        disconnect();
